...
export const printRenderStats = () => {
  ...
  console.debug('Time elapsed: ' + elapsed / 1000 + 's');
  console.debug('Total components tracked: ' + sortedComponents.length);
  
  console.group('Top 10 Most Frequently Rendered Components');
  sortedComponents.slice(0, 10).forEach(([name, stats]) => {
    console.debug(name + ': ' + stats.renderCount + ' renders (' + stats.totalRenderTime + 'ms total, ' + stats.totalRenderTime / stats.renderCount + 'ms avg)');
  });
  console.groupEnd();
  ...
  sortedComponents
    ...
    .slice(0, 10)
    .forEach(([name, stats]) => {
      console.debug(name + ': ' + stats.totalRenderTime / stats.renderCount + 'ms avg (' + stats.renderCount + ' renders)');
    });
  console.groupEnd();
  ...
  try {
    ...
    console.debug('Statistics saved to localStorage. Use the visualization tool to view them.');
  } catch (error) {
...
export const resetRenderStats = () => {
  ...
  console.debug('[RenderTracking] Statistics reset');
};
