...
export const analyzeModuleUsage = stats => {
  ...
  const packages = Object.entries(packageSizes).map(([name, size]) => ({
    name,
    size
  })).sort((a, b) => b.size - a.size);
...
const findDuplicatePackages = modules => {
  ...
  return Object.entries(packagePaths).filter(([_, paths]) => paths.size > 1).map(([name, paths]) => ({
    name,
    count: paths.size,
...
export const analyzeChunkSplitting = stats => {
  ...
  const chunkBudgets = Object.entries(BUNDLE_SIZE_BUDGETS).filter(([key]) => key !== 'total').map(([type, budget]) => {
...
export const generateOptimizationRecommendations = (moduleAnalysis, chunkAnalysis) => {
  ...
  if (moduleAnalysis.packages.length > 0) {
    const largePackages = moduleAnalysis.packages.filter(pkg => pkg.size > 100 * 1024) // More than 100KB
    .slice(0, 5);
    if (largePackages.length > 0) {
      recommendations.push({
        ...
        items: largePackages.map(pkg => ({
          ...
          suggestions: [`Consider replacing ${pkg.name} with a smaller alternative`, `Import only necessary parts of ${pkg.name} instead of the whole library`, `Use dynamic imports to load ${pkg.name} only when needed`]
        }))
      });
    }
  }
  ...
  if (moduleAnalysis.duplicates && moduleAnalysis.duplicates.length > 0) {
    recommendations.push({
      ...
      items: moduleAnalysis.duplicates.map(dup => ({
        name: dup.name,
        count: dup.count,
        suggestions: [`Deduplicate ${dup.name} by enforcing a single version in package.json`, `Use webpack's resolve.alias to force a single version`, `Update dependencies to use compatible versions`]
      }))
    });
  }
  ...
  if (chunkAnalysis.budgets) {
    ...
    if (chunkAnalysis.budgets.initial.exceeds) {
      recommendations.push({
        ...
        suggestions: ['Move non-critical components to lazy-loaded chunks', 'Implement code splitting for routes', 'Reduce the size of vendor bundle by importing only necessary dependencies', 'Use dynamic imports for heavy features']
      });
    }

    // Chunk-specific budgets
    const oversizedChunks = chunkAnalysis.budgets.chunkBudgets.flatMap(budget => budget.chunks).filter(chunk => chunk.exceeds);
    if (oversizedChunks.length > 0) {
      recommendations.push({
        ...
        items: oversizedChunks.map(chunk => ({
          ...
          suggestions: [`Split ${chunk.name} into smaller chunks`, `Optimize imports within ${chunk.name}`, `Consider code splitting within ${chunk.name}`]
        }))
      });
    }
  }
      ...
      try {
        ...
        if (!window.__BUNDLE_TRACKER__.modules[moduleId]) {
          ...
          for (const heavyImport of heavyImports) {
            if (moduleText.includes(`from '${heavyImport.name}'`) || moduleText.includes(`require('${heavyImport.name}')`)) {
              if (size > heavyImport.warnSize) {
                console.warn(`[BundleTracker] Heavy import detected: ${heavyImport.name} (${size.toFixed(2)}KB). ` + `Consider dynamic import or tree-shaking.`);
              }
...
export const trackImport = (importName, warnSizeKb = 50) => {
  ...
  if (!importSizes[importName]) {
    ...
    if (warnSizeKb > 30) {
      console.warn(`[BundleTracker] Heavy import: ${importName} (estimated: ${warnSizeKb}KB). ` + `Consider using dynamic import or code-splitting.`);
    }
  }
  return importName;
};
...
export const importByCapability = options => {
  ...
  const getDeviceCapability = () => {
    ...
    const isHighEnd = memory >= 4 && (effectiveType === '4g' || !effectiveType) && !connection.saveData;
    const isLowEnd = memory <= 2 || ['slow-2g', '2g', '3g'].includes(effectiveType) || connection.saveData;
...
export const analyzeUnusedImports = stats => {
  ...
  const unusedImports = stats.modules.filter(module => module.usedExports && module.usedExports.length === 0 &&
  // Only include modules from our source code, not node_modules
  !module.name.includes('node_modules') && module.name.match(/\.(js|jsx|ts|tsx)$/)).map(module => ({
    name: module.name,
    size: module.size,
...
export const generateTreeShakingRecommendations = stats => {
  ...
  const nodeModules = stats.modules.filter(module => module.name.includes('node_modules') && module.size > 20 * 1024 // More than 20KB
  ).map(module => {
  ...
  const packages = Object.values(packageGroups).sort((a, b) => b.totalSize - a.totalSize);
...
export const generateBundleOptimizationReport = () => {
  const report = window.__BUNDLE_TRACKER__ ? {
    totalSize: Object.values(window.__BUNDLE_TRACKER__.modules).reduce((sum, mod) => sum + mod.size, 0),
    moduleCount: Object.keys(window.__BUNDLE_TRACKER__.modules).length,
    heavyImports: Object.entries(window.__BUNDLE_TRACKER__.imports).map(([name, data]) => ({
      name,
      ...data
    })).sort((a, b) => b.size - a.size)
  } : {
