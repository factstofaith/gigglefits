...
const A11yDataChart = (props) => {
  ...
  const handleKeyNavigation = React.useCallback((event, currentIndex) => {
    const handleArrowKey = getArrowKeyHandler(currentIndex, navigationGrid, (nextIndex) => {
  ...
  const generateTextRepresentation = React.useCallback(() => {
    ...
    vizData.forEach((dataPoint) => {
      ...
      const percentage = max > min ? Math.round((dataPoint.value - min) / (max - min) * 100) : 0;
    ...
    return <Paper elevation={3} sx={{
      ...
      mb: 2
        ...
        <Tabs value={a11yView} onChange={handleViewChange} variant="fullWidth" aria-label="Visualization view options">
          ...
          <Tab value="visual" label="Visual" icon={<InfoIcon />} iconPosition="start" aria-label="Visual chart view" />
  ...
  return <div ref={componentRef} tabIndex={keyboardNavigationActive ? 0 : -1} aria-label={accessibilityLabel || `chart visualization`} style={{
    outline: keyboardNavigationActive ? '2px solid #1976d2' : 'none'
      ...
      {a11yView === 'visual' ? <DataChart data={vizData} {...otherProps} aria-hidden={a11yView !== 'visual'} /> : a11yView === 'table' ? renderTableView() : renderTextView()}
