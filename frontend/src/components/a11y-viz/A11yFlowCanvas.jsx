...
const A11yFlowCanvas = (props) => {
  ...
  const handleKeyNavigation = React.useCallback((event, currentNode) => {
    ...
    const handleArrowKey = getArrowKeyHandler(currentNode, connections, (nextNodeId) => {
...
`;
    ...
    return <Paper elevation={3} sx={{
      ...
      mb: 2
        ...
        <Tabs value={a11yView} onChange={handleViewChange} variant="fullWidth" aria-label="Visualization view options">
          ...
          <Tab value="visual" label="Visual" icon={<InfoIcon />} iconPosition="start" aria-label="Visual flow view" />
  ...
  return <div ref={componentRef} tabIndex={keyboardNavigationActive ? 0 : -1} aria-label={accessibilityLabel || `flow visualization`} style={{
    outline: keyboardNavigationActive ? '2px solid #1976d2' : 'none'
      ...
      {a11yView === 'visual' ? <FlowCanvas data={vizData} {...otherProps} aria-hidden={a11yView !== 'visual'} /> : a11yView === 'table' ? renderTableView() : renderTextView()}
