...
import { AppBar, Toolbar, Typography, Box, Button, IconButton, Drawer, List, ListItem, ListItemIcon, ListItemText, Collapse, Divider, Tooltip, Badge, Avatar, Menu, MenuItem, useTheme, useMediaQuery } from '@mui/material';
import { Menu as MenuIcon, Dashboard as DashboardIcon, Apps as AppsIcon, Storage as StorageIcon, Group as GroupIcon, Settings as SettingsIcon, ExpandLess, ExpandMore, Notifications as NotificationsIcon, Help as HelpIcon, AccountCircle, Business as BusinessIcon, Security as SecurityIcon, Logout as LogoutIcon, Inbox as InboxIcon, Dvr as DvrIcon, Assessment as AssessmentIcon, Tune as TuneIcon, Search as SearchIcon } from '@mui/icons-material';
...
const AdminNavigationBar = ({
  ...
  const renderDrawer = () => <Drawer variant={isMobile ? 'temporary' : 'persistent'} open={drawerOpen} onClose={toggleDrawer} sx={{
    width: 260,
    flexShrink: 0,
    '& .MuiDrawer-paper': {
      width: 260,
      ...
      p: 2
        ...
        <Box display="flex" alignItems="center" justifyContent="space-between">
          ...
          {isMobile && <IconButton onClick={toggleDrawer}>
              <MenuIcon />
            </IconButton>}
      ...
      overflow: 'auto'
        ...
        <List component="nav">
          ...
          pt: 2
            ...
            <Avatar alt={user?.name || 'Admin'} src={user?.avatarUrl} sx={{
            mr: 2
          }} />

            <ListItemText primary={user?.name || 'Admin User'} secondary={user?.role || 'Administrator'} />
          ...
          const hasVisibleChildren = section.children?.some(child => (!child.permission || hasPermission(child.permission)) && (!child.featureFlag || isFeatureEnabled(child.featureFlag)));
          ...
          return <React.Fragment key={section.id}>
                {section.expandable ? <>
                    <ListItem button onClick={() => toggleSection(section.id)} selected={expandedSection === section.id}>
                    ...
                    if (child.permission && !hasPermission(child.permission) || child.featureFlag && !isFeatureEnabled(child.featureFlag)) {
                      return null;
                    }
                    return <ListItem button key={`${section.id}-child-${childIndex}`} onClick={() => handleNavigation(child.path)} selected={isRouteActive(child.path)} sx={{
                      pl: 4
                    }}>
                  ...
                  </> : <ListItem button onClick={() => handleNavigation(section.path)} selected={isRouteActive(section.path)}>
  ...
  return <>
      ...
      color: theme.palette.text.primary
        ...
        <Toolbar>
          ...
          {hasPermission('use:global_search') && <Tooltip title="Search">
          ...
          {/* Notifications */}
          {hasPermission('view:notifications') && <Tooltip title="Notifications">
          ...
          <Tooltip title="Account">
            <IconButton edge="end" color="inherit" onClick={handleProfileMenuOpen} sx={{
            ml: 1
          }}>
      ...
      {/* Profile menu */}
      <Menu anchorEl={menuAnchorEl} id="account-menu" keepMounted open={Boolean(menuAnchorEl)} onClose={handleProfileMenuClose} transformOrigin={{
      horizontal: 'right',
      vertical: 'top'
    }} anchorOrigin={{
      horizontal: 'right',
      vertical: 'bottom'
        ...
        {hasPermission('manage:security_settings') && <MenuItem onClick={() => {
        handleProfileMenuClose();
      ...
      {/* Notifications menu */}
      <Menu anchorEl={notificationAnchorEl} id="notification-menu" keepMounted open={Boolean(notificationAnchorEl)} onClose={handleNotificationMenuClose} transformOrigin={{
      horizontal: 'right',
      vertical: 'top'
    }} anchorOrigin={{
      horizontal: 'right',
      ...
      style: {
        ...
        p: 0
            ...
            <ListItem button>
              ...
              <ListItemText primary="Sample Notification" secondary="This is a placeholder notification. View all to see actual notifications." />
...
export default withErrorBoundary(AdminNavigationBar, {
  boundary: 'AdminNavigationBar',
  fallback: ({
    error,
    resetError
  }) => <Box sx={{
    ...
    width: '100%'
      ...
      p: 2
                    ...
                    <Button variant="contained" color="primary" onClick={resetError} startIcon={<RefreshIcon />}>
                      Reload Dashboard
                    </Button>
                  </Paper>
                </Box>
