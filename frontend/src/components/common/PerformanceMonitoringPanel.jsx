...
} from '../../utils/performance/performanceBudget';
 ...
 */
const PerformanceMonitoringPanel = ({ onClose }) => {
  ...
  useEffect(() => {
    if (monitoringEnabled && !disableMonitoring) {
      ...
      const intervalId = setInterval(() => {
        ...
        setViolations(checkAllBudgets({ logToConsole: false }));
      }, 2000);
  ...
  const totalViolations = Object.values(violations).reduce(
    (sum, categoryViolations) => sum + categoryViolations.length, 
    0
  );
      ...
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Box>
          <FormControlLabel
            ...
            label={monitoringEnabled ? "Monitoring Active" : "Monitoring Inactive"}
          />
          {totalViolations > 0 && (
        ...
        {tabValue === 1 && (
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                {Object.entries(metrics.interactions)
                  .sort(([, a], [, b]) => b.averageTime - a.averageTime)
                  .map(([name, data]) => {
        ...
        {tabValue === 2 && (
          <TableContainer component={Paper}>
            <Table size="small">
              ...
              <TableBody>
                {Object.entries(metrics.resources)
                  .sort(([, a], [, b]) => b.averageTime - a.averageTime)
                  .map(([name, data]) => {
                ...
                {Object.entries(metrics.navigation).map(([url, data]) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell 
                        align="right"
                        sx={{ color: violations.includes('domInteractive') ? 'warning.main' : 'inherit' }}
                      >
                        {data.domInteractive.toFixed(2)}
                      </TableCell>
                      <TableCell 
                        align="right"
                        sx={{ color: violations.includes('domComplete') ? 'warning.main' : 'inherit' }}
                      >
                        {data.domComplete.toFixed(2)}
                      </TableCell>
                      <TableCell 
                        align="right"
                        sx={{ color: violations.includes('firstPaint') ? 'warning.main' : 'inherit' }}
                      >
                        {data.firstPaint.toFixed(2)}
                      </TableCell>
                      <TableCell 
                        align="right"
                        sx={{ color: violations.includes('firstContentfulPaint') ? 'warning.main' : 'inherit' }}
                      >
                        {data.firstContentfulPaint.toFixed(2)}
                      </TableCell>
                      <TableCell 
                        align="right"
                        sx={{ color: violations.includes('load') ? 'warning.main' : 'inherit' }}
                      >
                        {data.load.toFixed(2)}
                      </TableCell>
                    </TableRow>
                                ...
                                >
                                  ...
                                  <TableCell align="right">
                                    {category === 'bundleSizes' 
                                      ? `${(violation.value / 1024).toFixed(1)} KB`
                                      : `${violation.value.toFixed(1)} ms`}
                                  </TableCell>
                                  <TableCell align="right">
                                    {category === 'bundleSizes' 
                                      ? `${(violation.budget / 1024).toFixed(1)} KB`
                                      : `${violation.budget.toFixed(1)} ms`}
                                  </TableCell>
                                  <TableCell align="right">
                                    {category === 'bundleSizes' 
                                      ? `${(violation.overage / 1024).toFixed(1)} KB`
                                      : `${violation.overage.toFixed(1)} ms`}
                                  </TableCell>
        ...
        <DialogContent>
          <Box sx={{ my: 2 }}>
            ...
            <FormControlLabel
              control={
                <Switch 
                  checked={monitoringConfig.trackComponents} 
                  onChange={handleConfigChange('trackComponents')}
                />
              }
              label="Track Component Render Times"
            />
            <FormControlLabel
              control={
                <Switch 
                  checked={monitoringConfig.trackInteractions} 
                  onChange={handleConfigChange('trackInteractions')}
                />
              }
            ...
            <FormControlLabel
              control={
                <Switch 
                  checked={monitoringConfig.trackNavigation} 
                  onChange={handleConfigChange('trackNavigation')}
                />
              }
              label="Track Navigation Performance"
            />
            <FormControlLabel
              control={
                <Switch 
                  checked={monitoringConfig.trackResources} 
                  onChange={handleConfigChange('trackResources')}
                />
              }
              label="Track Resource Load Times"
            />
            <FormControlLabel
              control={
                <Switch 
                  checked={monitoringConfig.logToConsole} 
                  onChange={handleConfigChange('logToConsole')}
                />
              }
              label="Log Performance Data to Console"
            />
          </Box>
        </DialogContent>
        <DialogActions>
