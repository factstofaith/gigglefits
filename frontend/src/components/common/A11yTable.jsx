...
const A11yTable = forwardRef(({
  ...
  return (
    <TableContainer 
      component={Paper} 
      className={`a11y-table-container ${className}`}
      style={style}
      {...rest}
    >
      {a11yCaption && (
        <caption id={captionId} className="a11y-table-caption">
          {a11yCaption}
        </caption>
      )}
      
      {a11ySummary && (
        <div id={summaryId} className="a11y-table-summary sr-only">
          {a11ySummary}
        </div>
      )}
      ...
      <Table 
        ...
        aria-describedby={a11ySummary ? summaryId : undefined}
      >
        <TableHead>
          <TableRow>
            {columns.map(column => {
                ...
                >
                  {column.sortable !== false && onSort ? (
                    <TableSortLabel
                      ...
                      aria-label={a11yColumnHeaders[column.id] || `Sort by ${column.label}`}
                    >
                      {column.label}
                    </TableSortLabel>
                  ) : (
