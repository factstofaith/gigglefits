  ...
  return <Paper elevation={4} sx={{
    ...
    borderRadius: 2
      ...
      alignItems: 'center'
        ...
        <Box>
          <IconButton size="small" color="inherit" onClick={onClose} aria-label="Close tour">
      ...
      py: 0.5
        ...
        <LinearProgress variant="determinate" value={step / totalSteps * 100} sx={{
        height: 4,
        mb: 1
      }} />
      ...
      borderColor: 'divider'
        ...
        <Button onClick={onSkip} size="small" sx={{
        minWidth: 'auto'
      }}>
        ...
        gap: 1
          ...
          {step > 1 && <Button onClick={onPrev} variant="outlined" size="small" startIcon={<PrevIcon />}>

              Back
            </Button>}

          
          <Button onClick={onNext} variant="contained" color="primary" size="small" endIcon={step < totalSteps ? <NextIcon /> : <CheckCircleIcon />}>
...
const GuidedTour = ({
  ...
  if (isActive && currentTour && currentStepData) {
    return <Portal>
        ...
        {currentStepData.overlay && <Backdrop open={true} sx={{
        zIndex: 9997,
        ...
        {currentStepData.elementSelector && <TourSpotlight elementSelector={currentStepData.elementSelector} visible={isActive} pulse={currentStepData.pulse} />}


        
        {/* Tooltip with step information */}
        <Fade in={isActive}>
          <TourTooltip step={currentStep} totalSteps={currentTour.steps.length} title={currentTour.title} content={currentStepData.content} position={tooltipPosition} onNext={nextStep} onPrev={prevStep} onClose={closeTour} onSkip={skipTour} />
  ...
  return <TourSelectionDialog open={showTourSelector} onClose={() => setShowTourSelector(false)} tours={tours} onSelectTour={startTour} />;
};
