...
import { Box, Typography, Paper, Collapse, Button, IconButton, Tabs, Tab, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Tooltip, LinearProgress, Chip, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
import { getPerformanceMetrics, getPerformanceBudget, checkAllBudgets, getViolations, clearViolations, generateBudgetReport } from "@/utils/performance";
...
const PerformanceBudgetPanel = ({
  ...
  return <>
      ...
      ...positionStyles[position]
        ...
        backgroundColor: severity === 'error' ? '#ffebee' : severity === 'warning' ? '#fff8e1' : '#e8f5e9',
        padding: isOpen ? 2 : 1,
        cursor: 'pointer',
        ...
        borderBottomRightRadius: isOpen ? 0 : 4
          ...
          alignItems: 'center'
            ...
            {severity === 'error' ? <ErrorIcon color="error" sx={{
            mr: 1
          }} /> : severity === 'warning' ? <WarningIcon color="warning" sx={{
            mr: 1
          ...
          <Box>
            {isOpen && <IconButton size="small" onClick={e => {
            e.stopPropagation();
            checkBudgets();
          }}>
        ...
        p: 2
          ...
          mb: 2
              ...
              <Button variant="outlined" size="small" startIcon={<RefreshIcon />} onClick={checkBudgets}>

                Refresh
              </Button>
              
              <Box>
                <Button variant="outlined" size="small" startIcon={<BarChartIcon />} onClick={handleShowReport} sx={{
              mr: 1
            }}>

                  Report
                </Button>
                <Button variant="outlined" color="error" size="small" onClick={handleClearViolations}>
          ...
          mb: 2
            ...
            flexWrap: 'wrap'
                ...
                <ViolationChip label="Components" count={violations.components.length} onClick={() => {
              setActiveTab(0);
              toggleExpanded('components');
            }} />

                <ViolationChip label="Interactions" count={violations.interactions.length} onClick={() => {
              setActiveTab(0);
              toggleExpanded('interactions');
            }} />

                <ViolationChip label="Resources" count={violations.resources.length} onClick={() => {
              setActiveTab(0);
              toggleExpanded('resources');
            }} />

                <ViolationChip label="Navigation" count={violations.navigation.length} onClick={() => {
              setActiveTab(0);
              toggleExpanded('navigation');
            }} />

                <ViolationChip label="Bundle Sizes" count={violations.bundleSizes.length} onClick={() => {
              setActiveTab(0);
        ...
        <Box>
            ...
            py: 3
              ...
              mt: 1
                  ...
                  </Box> : Object.entries(violations).map(([category, categoryViolations]) => categoryViolations.length > 0 && <Box key={category} sx={{
            mb: 2
          }}>
                        <Box sx={{
              display: 'flex',
              ...
              borderColor: 'divider'
                          ...
                          <IconButton size="small">
                            {expanded[category] ? <ExpandMoreIcon style={{
                ...
                mt: 1
                      ...
                      return <TableRow key={`${category}-${idx}`} sx={{
        ...
        // Budgets tab
        <Box>
                {Object.entries(getPerformanceBudget()).map(([category, budgets]) => <Box key={category} sx={{
            mb: 2
          }}>
                    <Box sx={{
              display: 'flex',
      ...
      <Dialog open={reportDialogOpen} onClose={() => setReportDialogOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogTitle>
          ...
          <IconButton aria-label="close" onClick={() => setReportDialogOpen(false)} sx={{
          position: 'absolute',
          right: 8,
          top: 8
        }}>
        ...
        <DialogContent dividers>
          ...
          justifyContent: 'flex-end'
            ...
            <Button startIcon={<DownloadIcon />} variant="outlined" onClick={handleDownloadReport}>

              Download Report
            </Button>
          </Box>
          
          <iframe srcDoc={reportHtml} style={{
          width: '100%',
          height: '70vh',
          border: 'none'
...
const ViolationChip = ({
  ...
  return <Tooltip title={`${count} ${label} violation${count !== 1 ? 's' : ''}`}>
      <Chip label={`${label}: ${count}`} color={color} icon={icon} onClick={onClick} sx={{
      cursor: 'pointer'
    }} />
