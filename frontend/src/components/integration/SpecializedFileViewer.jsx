...
const CsvViewer = ({ content, fileName }) => {
  ...
  if (!content || data.length === 0) {
    return (
      <Alert severity="info">No CSV data to display</Alert>
    );
  }
...
const JsonViewer = ({ content }) => {
  ...
  const formatValue = useCallback((value) => {
    ...
    switch (typeof value) {
      case 'boolean':
        return <span style={{ color: '#0d6efd' }}>{value.toString()}</span>;
      case 'number':
      ...
      case 'string':
        ...
        return <span style={{ color: '#dc3545' }}>{`"${displayValue}"`}</span>;
      default:
        return String(value);
    }
  }, []);
          ...
          {isExpanded && (
            <Box sx={{ ml: 2 }}>
              {Object.entries(data).map(([key, value], index) => {
                ...
                return (
                  <Box key={childPath} sx={{ display: 'flex', flexDirection: 'column', mt: 0.5 }}>
  ...
  if (!content) {
    return (
      <Alert severity="info">No JSON content to display</Alert>
    );
  }
  
  return (
    <Box sx={{ fontFamily: 'monospace', fontSize: '0.9rem', p: 2, bgcolor: 'background.paper' }}>
...
const XmlViewer = ({ content }) => {
      ...
      <Box sx={{ fontFamily: 'monospace', whiteSpace: 'pre-wrap' }}>
        {lines.map((line, index) => {
          ...
          return (
            ...
            >
              {tag && !isClosing && !line.includes('/>') && !line.startsWith('<?') && (
                <IconButton size="small" onClick={() => toggleElement(tag)}>
                  {expandedElements[tag] ? 
  ...
  if (!content) {
    return (
      <Alert severity="info">No XML content to display</Alert>
    );
  }
...
const PdfViewer = ({ file, url }) => {
  ...
  if (!src) {
    return (
      <Alert severity="info">No PDF document to display</Alert>
    );
  }
...
const TextViewer = ({ content }) => {
  ...
  if (!content) {
    return (
      <Alert severity="info">No text content to display</Alert>
    );
  }
...
const SpecializedFileViewer = ({
  ...
  const renderViewer = useCallback(() => {
    ...
    if (!type) {
      return (
        <Alert severity="info">Unable to determine file type</Alert>
      );
    }
        ...
        <Box sx={{ mb: 2 }}>
          ...
          >
            <Tab 
              label="Preview" 
              value="preview" 
              icon={detectedType?.category === 'data' ? <TableIcon fontSize="small" /> : <DocumentIcon fontSize="small" />}
              iconPosition="start"
            />
            <Tab 
              label="Raw" 
...
export default SpecializedFileViewer;