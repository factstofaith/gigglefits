      ...
      case DATA_TYPES.URL:
        return (
          <Box sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 0.5 }}>
            ...
            {stats.uniqueCount !== undefined && stats.nonNullCount !== undefined && (
              <Typography variant="caption" display="block">
      ...
      default:
        return (
          <Box sx={{ fontSize: '0.75rem', color: 'text.secondary', mt: 0.5 }}>
            ...
            {stats.nullPercentage !== undefined && (
              <Typography variant="caption" display="block">
  ...
  return (
    <TableRow hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
      <TableCell component="th" scope="row">
        {field.name}
        {field.isPrimaryKeyCandidate && (
          <Tooltip title="Potential Primary Key">
            <Chip 
              label="PK" 
              size="small" 
              color="primary" 
      ...
      <TableCell>
        {isEditing ? (
          ...
          <Chip 
            label={field.type} 
            size="small"
            color={field.confidence >= CONFIDENCE_THRESHOLD.HIGH ? "primary" : "default"}
          />
        )}
        
        {field.alternativeTypes && field.alternativeTypes.length > 0 && !isEditing && (
          <Box sx={{ mt: 0.5 }}>
            {field.alternativeTypes.slice(0, 2).map(alt => (
              <Chip
                key={alt.type}
                label={alt.type}
                size="small"
            ...
            {field.alternativeTypes.length > 2 && (
              <Chip
                label={`+${field.alternativeTypes.length - 2} more`}
                size="small"
                variant="outlined"
...
const SchemaInferenceViewer = ({
  ...
  const summary = useMemo(() => {
    if (!schema || !schema.fields) {
      return { total: 0, highConfidence: 0, mediumConfidence: 0, lowConfidence: 0 };
    }
    ...
    const mediumConfidence = schema.fields.filter(f => 
      f.confidence >= CONFIDENCE_THRESHOLD.MEDIUM && f.confidence < CONFIDENCE_THRESHOLD.HIGH
    ).length;
  ...
  return (
    <Paper sx={{ maxHeight }}>
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {schema?.recordCount 
              ? `Inferred from ${schema.sampledRecordCount} of ${schema.recordCount} records`
              : 'No schema detected'
            }
          </Typography>
        </Box>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <Tooltip title="Regenerate Schema">
            <IconButton 
              ...
              disabled={isInferring || !data || data.length === 0}
            >
              <RefreshIcon />
            </IconButton>
          </Tooltip>
          ...
          <Tooltip title="Export Schema as JSON Schema">
            <IconButton 
              ...
              disabled={!schema || !schema.fields || schema.fields.length === 0}
            >
              <DownloadIcon />
            </IconButton>
          </Tooltip>
          ...
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} sm={6}>
              ...
              <Slider
                ...
                min={100}
                max={Math.max(1000, data?.length || 0)}
                step={100}
                valueLabelDisplay="auto"
                onChange={(_, value) => handleSettingChange('sampleSize', value)}
            ...
            <Grid item xs={12} sm={6}>
              <FormControlLabel
                control={
                  <Switch
                    checked={settings.detectSpecialTypes}
                    onChange={e => handleSettingChange('detectSpecialTypes', e.target.checked)}
                    disabled={isInferring}
                  />
                }
                label="Detect Special Types (Email, URL, Date, etc.)"
              />
            </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <FormControlLabel
                ...
                label="Include Field Statistics"
              />
            </Grid>
          </Grid>
        </Box>
        ...
        <Box sx={{ p: 2, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Grid container spacing={2}>
            ...
            <Grid item>
              <Chip 
                icon={<ValidIcon />}
                label={`${summary.highConfidence} High Confidence`}
                color="success"
                variant="outlined"
              />
            </Grid>
            <Grid item>
              <Chip 
                icon={<InfoIcon />}
                label={`${summary.mediumConfidence} Medium Confidence`}
                color="info"
                variant="outlined"
              />
            </Grid>
            <Grid item>
              <Chip 
                icon={<WarningIcon />}
                label={`${summary.lowConfidence} Low Confidence`}
                color="warning"
                variant="outlined"
              />
            </Grid>
      ...
      {/* Schema table */}
      {schema && schema.fields && schema.fields.length > 0 ? (
        <TableContainer sx={{ maxHeight: maxHeight - 150 }}>
