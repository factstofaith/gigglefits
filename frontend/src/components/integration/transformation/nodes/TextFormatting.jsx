...
const validationSchema = Yup.object().shape({
  ...
  operation: Yup.string().required('Operation is required').oneOf([
    'uppercase', 'lowercase', 'capitalize', 'trim', 'replace', 'substring', 
    'padStart', 'padEnd', 'remove', 'concat', 'template'
  ], 'Invalid operation'),
...
const TEXT_OPERATIONS = [
  ...
  { value: 'capitalize', label: 'Capitalize', description: 'Capitalize the first letter of each word' },
  { value: 'trim', label: 'Trim', description: 'Remove whitespace from both ends of the text' },
  { value: 'replace', label: 'Replace', description: 'Replace occurrences of a substring' },
  { value: 'substring', label: 'Substring', description: 'Extract a portion of the text' },
  { value: 'padStart', label: 'Pad Start', description: 'Pad the start of the text to a specified length' },
  { value: 'padEnd', label: 'Pad End', description: 'Pad the end of the text to a specified length' },
  { value: 'remove', label: 'Remove', description: 'Remove all occurrences of a substring' },
  { value: 'concat', label: 'Concatenate', description: 'Combine multiple fields into one' },
  { value: 'template', label: 'Template', description: 'Format text using a template with placeholders' }
];
...
const performTextFormatting = (input, config) => {
  ...
  if (input === null || input === undefined || 
      (typeof input === 'string' && treatAsNull && treatAsNull.includes(input))) {
    return '';
  }
...
const TextFormattingConfigPanel = ({ 
  ...
  const handleAdditionalFieldsChange = useCallback((fields) => {
    onChange({ ...config, additionalFields: fields });
  }, [config, onChange]);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            helperText={validationState.errors.inputField || 'Field to format'}
            disabled={disabled}
            InputProps={{ readOnly }}
            fullWidth
            required
      ...
      {(config.operation === 'replace' || config.operation === 'remove') && (
        <Grid container spacing={2}>
          <Grid item xs={12} sm={config.operation === 'replace' ? 6 : 12}>
            <TextField
              ...
              onChange={handleChange}
              error={Boolean(validationState.errors.searchValue)}
              helperText={validationState.errors.searchValue || 'Text to search for'}
              disabled={disabled}
              InputProps={{ readOnly }}
              fullWidth
              required
          ...
          {config.operation === 'replace' && (
            <Grid item xs={12} sm={6}>
              <TextField
                ...
                onChange={handleChange}
                error={Boolean(validationState.errors.replaceValue)}
                helperText={validationState.errors.replaceValue || 'Text to replace with'}
                disabled={disabled}
                InputProps={{ readOnly }}
                fullWidth
                required
        ...
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              label="Start Index"
              type="number"
              value={config.startIndex !== undefined ? config.startIndex : 0}
              onChange={handleChange}
              error={Boolean(validationState.errors.startIndex)}
              helperText={validationState.errors.startIndex || 'Starting character position'}
              disabled={disabled}
              InputProps={{ readOnly }}
              fullWidth
              required
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              type="number"
              value={config.endIndex !== null && config.endIndex !== undefined ? config.endIndex : ''}
              onChange={handleChange}
              error={Boolean(validationState.errors.endIndex)}
              helperText={validationState.errors.endIndex || 'Ending character position (leave empty for end of string)'}
              disabled={disabled}
              InputProps={{ readOnly }}
              fullWidth
            />
      ...
      {(config.operation === 'padStart' || config.operation === 'padEnd') && (
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <TextField
              name="length"
              label="Target Length"
              type="number"
              value={config.length !== undefined ? config.length : 10}
              onChange={handleChange}
              error={Boolean(validationState.errors.length)}
              helperText={validationState.errors.length || 'Length to pad to'}
              disabled={disabled}
              InputProps={{ readOnly }}
              fullWidth
              required
          ...
          <Grid item xs={12} sm={6}>
            <TextField
              ...
              onChange={handleChange}
              error={Boolean(validationState.errors.padChar)}
              helperText={validationState.errors.padChar || 'Character to pad with'}
              disabled={disabled}
              InputProps={{ readOnly }}
              fullWidth
      ...
      {config.operation === 'concat' && (
        <Box>
          <TextField
            name="separator"
            label="Separator"
            value={config.separator !== undefined ? config.separator : ' '}
            onChange={handleChange}
            error={Boolean(validationState.errors.separator)}
            helperText={validationState.errors.separator || 'Character(s) to separate fields with'}
            disabled={disabled}
            InputProps={{ readOnly }}
            fullWidth
            sx={{ mb: 2 }}
          ...
          <Box sx={{ mb: 1 }}>
            <FormControlLabel
              control={
                <Checkbox
                  checked={Boolean(config.additionalFields?.length)}
                  onChange={(e) => {
                    if (e.target.checked) {
          ...
          {Boolean(config.additionalFields?.length) && (
      ...
      {config.operation === 'template' && (
        <TextField
          ...
          onChange={handleChange}
          error={Boolean(validationState.errors.template)}
          helperText={validationState.errors.template || 'Template with {{value}} placeholders (e.g., "Name: {{value}}")'}
          disabled={disabled}
          InputProps={{ readOnly }}
          fullWidth
          required
      ...
      {['replace', 'remove'].includes(config.operation) && (
        <Box sx={{ mt: 1 }}>
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <FormControlLabel
                control={
                  <Switch
                    name="useRegex"
                    checked={Boolean(config.useRegex)}
...
const TextFormatting = (props) => {
  return (
    <TransformationNodeTemplate
      ...
      icon={TextFormatIcon}
      description="Formats and manipulates text values"
      configPanel={TextFormattingConfigPanel}
