...
import { Box, TextField, FormControl, InputLabel, Select, MenuItem, FormHelperText, Grid, Typography, Divider, Alert, Chip, IconButton, Switch, FormControlLabel, Button, List, ListItem, ListItemText, ListItemSecondaryAction, Paper } from '@mui/material';
...
const validationSchema = Yup.object().shape({
  ...
  cleansingOperations: Yup.array().of(Yup.object().shape({
    type: Yup.string().required('Operation type is required').oneOf(['trim', 'replace', 'normalize', 'removeNonAlphanumeric', 'removeDiacritics', 'removeHtml', 'removeEmojis', 'truncate', 'fixEncoding', 'mask', 'removeExtraSpaces', 'standardizeLineEndings', 'titleCase', 'lowercase', 'uppercase'], 'Invalid operation type'),
...
const CLEANSING_OPERATIONS = [{
  ...
  value: 'replace',
  label: 'Replace Text',
  description: 'Replace occurrences of a pattern with replacement text'
}, {
  value: 'normalize',
  label: 'Normalize Text',
  description: 'Standardize text by normalizing whitespace and removing control characters'
}, {
  ...
  description: 'Remove diacritical marks (accents)'
}, {
  value: 'removeHtml',
  label: 'Remove HTML',
  ...
  description: 'Replace part of the text with a mask character'
}, {
  value: 'removeExtraSpaces',
  label: 'Remove Extra Spaces',
  description: 'Replace multiple spaces with a single space'
}, {
  value: 'standardizeLineEndings',
  label: 'Standardize Line Endings',
  description: 'Convert all line endings to standard format'
}, {
  value: 'titleCase',
  label: 'Title Case',
...
const performDataCleansing = (input, config) => {
  ...
  if (input === null || input === undefined || treatEmptyAsNull && typeof input === 'string' && input.trim() === '') {
  ...
  cleansingOperations.forEach((operation) => {
    switch (operation.type) {
      ...
      case 'normalize':
        ...
        result = result.replace(/[\x00-\x1F\x7F-\x9F]/g, '') // Remove control characters
      ...
      case 'fixEncoding':
        ...
        result = result.replace(/Â/g, '') // Remove invisible character often from copy/paste
...
const CleansingOperationConfig = ({
  ...
  const operationErrors = useMemo(() => {
    ...
    const operationIndex = validationState.errors.cleansingOperations?.findIndex?.((err) => err.id === operation.id);
    return operationIndex >= 0 ? validationState.errors.cleansingOperations[operationIndex] || {} : {};
    ...
    mb: 2
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={10}>
          <FormControl fullWidth error={Boolean(operationErrors.type)} required>

            <InputLabel id={`operation-type-label-${operation.id}`}>Operation Type</InputLabel>
            <Select labelId={`operation-type-label-${operation.id}`} name="type" value={operation.type || ''} onChange={handleChange} disabled={disabled} readOnly={readOnly}>

              {CLEANSING_OPERATIONS.map((op) => <MenuItem key={op.value} value={op.value}>
                  {op.label}
                </MenuItem>)}

            </Select>
            <FormHelperText>
              {operationErrors.type || CLEANSING_OPERATIONS.find((op) => op.value === operation.type)?.description || 'Select an operation type'}
            </FormHelperText>
          </FormControl>
        </Grid>
        ...
        justifyContent: 'flex-end'
          ...
          <IconButton onClick={() => onRemoveOperation(operation.id)} disabled={disabled || readOnly} color="error" aria-label="Remove operation">
        ...
        {operation.type === 'replace' && <>
            <Grid item xs={12} sm={6}>
              <TextField name="pattern" label="Search Pattern" value={operation.pattern || ''} onChange={handleChange} error={Boolean(operationErrors.pattern)} helperText={operationErrors.pattern || 'Text or pattern to search for'} disabled={disabled} InputProps={{
            readOnly
          }} fullWidth required />

            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField name="replacement" label="Replacement" value={operation.replacement || ''} onChange={handleChange} error={Boolean(operationErrors.replacement)} helperText={operationErrors.replacement || 'Text to replace with (leave empty to remove)'} disabled={disabled} InputProps={{
            readOnly
          }} fullWidth />

            </Grid>
            <Grid item xs={12} sm={6}>
              <FormControlLabel control={<Switch name="useRegex" checked={Boolean(operation.useRegex)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Use Regular Expression" />

            </Grid>
            <Grid item xs={12} sm={6}>
              <FormControlLabel control={<Switch name="caseSensitive" checked={operation.caseSensitive !== false} // Default to true
          onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Case Sensitive" />

            </Grid>
          </>}

        
        {operation.type === 'truncate' && <Grid item xs={12}>
            <TextField name="maxLength" label="Maximum Length" type="number" value={operation.maxLength || ''} onChange={handleChange} error={Boolean(operationErrors.maxLength)} helperText={operationErrors.maxLength || 'Maximum number of characters to keep'} disabled={disabled} InputProps={{
          readOnly
        }} fullWidth required inputProps={{
          min: 1
        }} />
        ...
        {operation.type === 'mask' && <>
            <Grid item xs={12} sm={6}>
              <TextField name="pattern" label="Pattern to Mask" value={operation.pattern || ''} onChange={handleChange} error={Boolean(operationErrors.pattern)} helperText={operationErrors.pattern || 'Regular expression pattern for text to mask'} disabled={disabled} InputProps={{
            readOnly
          }} fullWidth required />

            </Grid>
            <Grid item xs={12} sm={6}>
              <TextField name="maskChar" label="Mask Character" value={operation.maskChar || ''} onChange={handleChange} error={Boolean(operationErrors.maskChar)} helperText={operationErrors.maskChar || 'Character to use for masking (e.g., *)'} disabled={disabled} InputProps={{
            readOnly
          }} fullWidth required inputProps={{
            maxLength: 1
          }} />
...
const DataCleansingConfigPanel = ({
  ...
  const handleUpdateOperation = useCallback((updatedOperation) => {
    const updatedOperations = config.cleansingOperations.map((op) => op.id === updatedOperation.id ? updatedOperation : op);
    onChange({
      ...config,
      cleansingOperations: updatedOperations
    });
  }, [config, onChange]);
  ...
  return <Box sx={{
    ...
    gap: 2
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <TextField name="inputField" label="Input Field" value={config.inputField || ''} onChange={handleChange} error={Boolean(validationState.errors.inputField)} helperText={validationState.errors.inputField || 'Field to cleanse'} disabled={disabled} InputProps={{
          readOnly
        }} fullWidth required />
      ...
      {/* List of operations */}
      {config.cleansingOperations?.map((operation) => <CleansingOperationConfig key={operation.id} operation={operation} onUpdateOperation={handleUpdateOperation} onRemoveOperation={handleRemoveOperation} validationState={validationState} disabled={disabled} readOnly={readOnly} />)}


      
      <Button startIcon={<AddIcon />} onClick={handleAddOperation} disabled={disabled || readOnly} variant="outlined" sx={{
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <FormControlLabel control={<Switch name="applyToNullValues" checked={Boolean(config.applyToNullValues)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Apply to Null Values" />

        </Grid>
        <Grid item xs={12} sm={6}>
          <FormControlLabel control={<Switch name="treatEmptyAsNull" checked={config.treatEmptyAsNull !== false} // Default to true
        onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Treat Empty String as Null" />

        </Grid>
        <Grid item xs={12}>
          <TextField name="nullReplacement" label="Null Replacement Value" value={config.nullReplacement || ''} onChange={handleChange} error={Boolean(validationState.errors.nullReplacement)} helperText={validationState.errors.nullReplacement || 'Value to use when input is null (leave empty to keep as null)'} disabled={disabled || !config.applyToNullValues} InputProps={{
          readOnly
        }} fullWidth />
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={3}>
          <FormControlLabel control={<Switch name="validateEmail" checked={Boolean(config.validateEmail)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Validate Email" />

        </Grid>
        <Grid item xs={12} sm={3}>
          <FormControlLabel control={<Switch name="validateUrl" checked={Boolean(config.validateUrl)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Validate URL" />

        </Grid>
        <Grid item xs={12} sm={3}>
          <FormControlLabel control={<Switch name="validatePhone" checked={Boolean(config.validatePhone)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Validate Phone" />

        </Grid>
        <Grid item xs={12} sm={3}>
          <FormControlLabel control={<Switch name="validatePostalCode" checked={Boolean(config.validatePostalCode)} onChange={handleCheckboxChange} disabled={disabled || readOnly} />} label="Validate Postal Code" />

        </Grid>
        
        {(config.validatePhone || config.validatePostalCode) && <Grid item xs={12}>
            <FormControl fullWidth error={Boolean(validationState.errors.region)} required={config.validatePhone || config.validatePostalCode}>

              <InputLabel id="region-label">Region</InputLabel>
              <Select labelId="region-label" name="region" value={config.region || 'US'} onChange={handleChange} disabled={disabled} readOnly={readOnly}>

                {REGIONS.map((region) => <MenuItem key={region.value} value={region.value}>
                    {region.label}
                  </MenuItem>)}
      ...
      <Box>
        ...
        <TextField label="Sample Input" value={sampleInput} onChange={handleSampleInputChange} fullWidth multiline rows={2} sx={{
        mb: 2
      }} />
        ...
        <Typography variant="body2" color="text.secondary">
          Applied operations: {config.cleansingOperations?.map((op) => CLEANSING_OPERATIONS.find((item) => item.value === op.type)?.label).join(' → ')}
        </Typography>
      </Box>
    </Box>;
};
...
const DataCleansing = (props) => {
  return <TransformationNodeTemplate title="Data Cleansing" icon={CleaningServicesIcon} description="Cleanses and standardizes text data" configPanel={DataCleansingConfigPanel} validationSchema={validationSchema} initialConfig={initialConfig} {...props} />;
};
