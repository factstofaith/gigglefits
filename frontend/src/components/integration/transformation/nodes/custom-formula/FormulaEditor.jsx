 ...
 */
const KEYWORDS = [
  'if', 'and', 'or', 'not', 'true', 'false', 'null', 'undefined'
];
 ...
 */
const FormulaEditor = ({ value, onChange, readOnly, validation, suggestions }) => {
  // Editor state
  ...
  const tokenize = useCallback((text) => {
    ...
    const processToken = () => {
      if (currentToken) {
        ...
        result.push({ type, value: currentToken });
        currentToken = '';
        currentType = null;
      }
    };
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      if (/\s/.test(char)) {
        processToken();
        result.push({ type: 'whitespace', value: char });
        continue;
      }
      ...
      if (/[+\-*/%^=!<>&|]/.test(char)) {
        ...
        if (['==', '!=', '>=', '<=', '&&', '||'].includes(twoCharOp)) {
          result.push({ type: 'operator', value: twoCharOp });
      ...
      result.push({ type: 'unknown', value: char });
    }
  ...
  useEffect(() => {
    ...
    if (word) {
      const filtered = suggestions.filter(suggestion => 
        suggestion.name.toLowerCase().startsWith(word.toLowerCase())
      );
    ...
    <div className="formula-editor-container">
      <div className="formula-editor-wrapper">
        <div className="syntax-highlight-container">
          ...
          <textarea
            ref={editorRef}
            className={`formula-editor-textarea ${validation?.isValid ? '' : 'has-errors'}`}
            value={value}
      ...
      {!validation?.isValid && validation?.errors?.length > 0 && (
        <div className="formula-editor-errors">
          {validation.errors.map((error, index) => (
            <div key={index} className="formula-editor-error">
              {error.message}
              {error.position !== undefined && (
                <span className="formula-editor-error-position"> at position {error.position}</span>
              )}
            </div>
          ))}
        </div>
      ...
      {showSuggestions && filteredSuggestions.length > 0 && (
        <div className="formula-editor-suggestions" ref={suggestionsRef}>
          {filteredSuggestions.slice(0, 7).map((suggestion, index) => (
            <div 
              key={index} 
              className={`formula-editor-suggestion ${index === activeSuggestion ? 'active' : ''}`}
                ...
                <div className="suggestion-examples">
                  ...
                  <ul className="examples-list">
                    {suggestion.examples.map((example, exampleIndex) => (
                      <li key={exampleIndex} className="example-item">{example}</li>
                    ))}
                  </ul>
                </div>
              )}
          ...
          {filteredSuggestions.length > 7 && (
            <div className="formula-editor-more-suggestions">
