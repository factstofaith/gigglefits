...
const FunctionCatalog = ({
  ...
  const categoriesToShow = useMemo(() => {
    if (searchTerm) {
      ...
      return Object.keys(functionsByCategory).filter(category => functionsByCategory[category].length > 0);
    }
    if (selectedCategory) {
  ...
  return <div className="function-catalog">
      <div className="function-catalog-search">
        <input type="text" value={searchTerm} onChange={handleSearchChange} placeholder="Search functions..." className="function-catalog-search-input" data-testid="function-search" />

      </div>
      
      <div className="function-catalog-categories">
        {Object.entries(FunctionCategories).map(([key, category]) => <button key={key} className={`function-catalog-category ${selectedCategory === category ? 'selected' : ''}`} onClick={() => handleCategoryClick(category)} disabled={searchTerm && !categoriesToShow.includes(category)} data-testid={`category-${key}`}>
      ...
      <div className="function-catalog-list">
        {categoriesToShow.map(category => <div key={category} className="function-catalog-category-section">
            <h4 className="function-catalog-category-title">{category}</h4>
            <div className="function-catalog-functions">
              {functionsByCategory[category]?.map(func => <div key={func.name} className="function-catalog-function" onClick={() => handleFunctionClick(func)} data-testid={`function-${func.name}`}>
              ...
              {(!functionsByCategory[category] || functionsByCategory[category].length === 0) && <div className="function-catalog-no-functions">
        ...
        {categoriesToShow.length === 0 && <div className="function-catalog-no-results">
