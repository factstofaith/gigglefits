...
const validationSchema = Yup.object().shape({
  ...
  formatString: Yup.string().when(['inputType', 'outputType'], {
    is: (inputType, outputType) => 
      (inputType === 'date' || outputType === 'date') ||
      (inputType === 'datetime' || outputType === 'datetime'),
...
const DATA_TYPES = [
  { value: 'string', label: 'String (Text)', description: 'Text data type' },
  { value: 'number', label: 'Number', description: 'Numeric data type (decimal or integer)' },
  { value: 'integer', label: 'Integer', description: 'Whole number without decimal places' },
  { value: 'float', label: 'Float', description: 'Decimal number' },
  { value: 'boolean', label: 'Boolean', description: 'True/False values' },
  { value: 'date', label: 'Date', description: 'Date without time information' },
  { value: 'datetime', label: 'DateTime', description: 'Date with time information' },
      ...
      case 'datetime':
        if (input instanceof Date) {
          stringValue = formatString 
            ? new Intl.DateTimeFormat('en-US', JSON.parse(formatString)).format(input)
            : input.toISOString();
        } else {
          const date = new Date(input);
          stringValue = formatString 
            ? new Intl.DateTimeFormat('en-US', JSON.parse(formatString)).format(date)
            : date.toISOString();
        }
        break;
      case 'array':
...
const DataTypeConversionConfigPanel = ({ 
  ...
  const isFormatStringRequired = useMemo(() => {
    return config.inputType === 'date' || config.inputType === 'datetime' ||
           config.outputType === 'date' || config.outputType === 'datetime';
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}>
          <TextField
            ...
            helperText={validationState.errors.inputField || 'Field to convert'}
            disabled={disabled}
            InputProps={{ readOnly }}
            fullWidth
            required
      ...
      {isFormatStringRequired && (
        <TextField
          name="formatString"
          label="Format String"
          value={config.formatString || ''}
          onChange={handleChange}
          error={Boolean(validationState.errors.formatString)}
          helperText={validationState.errors.formatString || 'For date/time formatting (Intl.DateTimeFormat options JSON)'}
          disabled={disabled}
          InputProps={{ readOnly }}
          fullWidth
          placeholder={getFormatPlaceholder()}
          required={isFormatStringRequired}
        />
      )}
      ...
      <TextField
        ...
        value={config.nullPlaceholder || ''}
        onChange={handleChange}
        error={Boolean(validationState.errors.nullPlaceholder)}
        helperText={validationState.errors.nullPlaceholder || 'Value to use when input is null (leave empty to keep as null)'}
        disabled={disabled}
        InputProps={{ readOnly }}
        fullWidth
      />
      ...
      <FormControl fullWidth error={Boolean(validationState.errors.errorHandling)}>
        ...
        <Select
          ...
          name="errorHandling"
          value={config.errorHandling || 'keepOriginal'}
          onChange={handleChange}
          disabled={disabled}
          readOnly={readOnly}
        >
...
const DataTypeConversion = (props) => {
  return (
    <TransformationNodeTemplate
      ...
      configPanel={DataTypeConversionConfigPanel}
