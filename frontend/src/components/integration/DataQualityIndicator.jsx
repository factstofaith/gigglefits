...
import { Box, Typography, Card, CardContent, Grid, Chip, LinearProgress, Tooltip, IconButton, Divider, Collapse, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Badge, Button, CircularProgress } from '@mui/material';

// Import icons
import { CheckCircle as CheckCircleIcon, Error as ErrorIcon, Warning as WarningIcon, Info as InfoIcon, ExpandMore as ExpandMoreIcon, ExpandLess as ExpandLessIcon, Assessment as AssessmentIcon, BugReport as BugReportIcon, Help as HelpIcon, Refresh as RefreshIcon, TrendingUp as TrendingUpIcon, TrendingDown as TrendingDownIcon, TrendingFlat as TrendingFlatIcon } from '@mui/icons-material';

// Import data quality utilities
import { QUALITY_DIMENSIONS, ISSUE_TYPES, SEVERITY, getQualityGrade } from "@/utils/dataQualityAnalyzer";
      ...
      <CardContent>
        ...
        mt: 1
          ...
          textAlign: 'left'
            ...
            <Chip size="small" label={color === 'success' ? 'Excellent' : color === 'info' ? 'Good' : color === 'warning' ? 'Fair' : 'Poor'} color={color} sx={{
            mt: 0.5
          }} />
  ...
  return <Card variant="outlined">
      ...
      '&:last-child': {
        ...
        <Grid container spacing={1} alignItems="center">
          ...
          <Grid item xs={12}>
            <LinearProgress variant="determinate" value={percentage} color={color} sx={{
            height: 8,
            borderRadius: 4
          }} />
  ...
  return <TableContainer component={Paper} variant="outlined">
      <Table size="small">
        ...
        <TableBody>
          ...
          return <TableRow key={fieldName} hover>
                ...
                <TableCell align="right">
                  {issueCount > 0 ? <Chip size="small" label={issueCount} color={issueCount > 10 ? "error" : issueCount > 5 ? "warning" : "info"} /> : <CheckCircleIcon color="success" fontSize="small" />}
      ...
      <CardContent>
        ...
        mb: 2
          ...
          <Grid container spacing={1}>
            <Grid item>
              <Chip icon={<ErrorIcon />} label={`Critical: ${criticalCount}`} size="small" color="error" variant={criticalCount > 0 ? "filled" : "outlined"} />

            </Grid>
            
            <Grid item>
              <Chip icon={<ErrorIcon />} label={`High: ${highCount}`} size="small" color="error" variant={highCount > 0 ? "filled" : "outlined"} />

            </Grid>
            
            <Grid item>
              <Chip icon={<WarningIcon />} label={`Medium: ${mediumCount}`} size="small" color="warning" variant={mediumCount > 0 ? "filled" : "outlined"} />

            </Grid>
            
            <Grid item>
              <Chip icon={<InfoIcon />} label={`Low: ${lowCount}`} size="small" color="info" variant={lowCount > 0 ? "filled" : "outlined"} />
        ...
        <Collapse in={isExpanded}>
          ...
          <TableContainer>
            <Table size="small">
              <TableBody>
                {Object.entries(issuesByType).map(([type, count]) => <TableRow key={type}>
                    <TableCell>{getIssueTypeName(type)}</TableCell>
                    <TableCell align="right">{count}</TableCell>
                  </TableRow>)}

                
                {Object.keys(issuesByType).length === 0 && <TableRow>
          ...
          {Array.isArray(issues) && issues.length > 0 && <>
            ...
            mb: 1
              ...
              {issues.slice(0, 5).map((issue, index) => <Box key={index} sx={{
            display: 'flex',
            alignItems: 'start',
            mb: 1,
            p: 1,
            ...
            bgcolor: 'background.default'
              ...
              ml: 1
                    ...
                    {issue.field && <Typography variant="caption" color="text.secondary">
                        Field: {issue.field}
                      </Typography>}
              ...
              {issues.length > 5 && <Typography variant="body2" color="text.secondary" sx={{
            mt: 1,
            textAlign: 'center'
          }}>
...
const DataQualityIndicator = ({
  ...
  return <Box sx={{
    ...
    p: 1
        ...
        <Grid item xs={12} sm={6} md={8}>
          <Card variant="outlined">
            ...
            pb: 1
              ...
              <Grid container spacing={1}>
                {Object.entries(dimensions).slice(0, 6).map(([dimension, score]) => <Grid item xs={6} sm={6} md={4} key={dimension}>
      ...
      {/* Issues summary */}
      {showIssues && issues && <Box sx={{
      mb: 2
    }}>
          <IssueSummaryCard issues={issues} isExpanded={expandedSections.issues} toggleExpanded={() => toggleSection('issues')} />

        </Box>}

      
      {/* Field metrics */}
      {showFieldMetrics && fieldMetrics && Object.keys(fieldMetrics).length > 0 && <Box sx={{
      mb: 2
    }}>
          <Card variant="outlined">
            <CardContent>
      ...
      <Box>
        <Card variant="outlined">
          <CardContent>
            ...
            {overallQuality < 0.9 ? <Box>
                {/* Completeness recommendations */}
                {dimensions[QUALITY_DIMENSIONS.COMPLETENESS] < 0.95 && <Box sx={{
              mb: 1,
              p: 1,
              bgcolor: 'background.default',
              borderRadius: 1
                ...
                {dimensions[QUALITY_DIMENSIONS.VALIDITY] < 0.95 && <Box sx={{
              mb: 1,
              p: 1,
              bgcolor: 'background.default',
              borderRadius: 1
                ...
                {dimensions[QUALITY_DIMENSIONS.CONSISTENCY] < 0.9 && <Box sx={{
              mb: 1,
              p: 1,
              bgcolor: 'background.default',
              borderRadius: 1
...
IssueSummaryCard.propTypes = {
  issues: PropTypes.oneOfType([PropTypes.array, PropTypes.object]),
