...
import { Box, Card, CardHeader, CardContent, Typography, Button, Grid, IconButton, List, ListItem, ListItemText, Select, MenuItem, TextField, Dialog, DialogTitle, DialogContent, DialogActions, FormControl, InputLabel, FormHelperText, LinearProgress, Tooltip, Divider, Paper } from '@mui/material';
import { Add as AddIcon, Delete as DeleteIcon, Refresh as RefreshIcon } from '@mui/icons-material';
import { withErrorBoundary } from "@/error-handling";
import { getFieldMappings, createFieldMapping, updateFieldMapping, deleteFieldMapping, discoverFields } from "@/services/integrationService";

// Available transformation types
const TRANSFORMATION_TYPES = [{
  value: 'direct',
  label: 'Direct (No transformation)'
}, {
  value: 'uppercase',
...
function FieldMappingEditor({
  ...
  return <Box>
      <Card>
        <CardHeader title="Field Mappings" subheader="Configure how fields are mapped from source to destination" action={<Box display="flex" gap={1}>
              ...
              <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={handleAddMapping}>
        ...
        {(loading.mappings || loading.sourceFields || loading.destinationFields) && <LinearProgress />}

        
        <CardContent>
          {mappings.length > 0 ? <List>
              {mappings.map((mapping, index) => <React.Fragment key={mapping.id}>
                  {index > 0 && <Divider />}
                  <ListItem secondaryAction={<IconButton edge="end" onClick={() => handleDeleteMapping(mapping.id)} aria-label="Delete mapping">

                        <DeleteIcon />
                      </IconButton>}>


                    <ListItemText primary={<Box display="flex" alignItems="center" gap={1} sx={{
                cursor: 'pointer'
                          ...
                          {mapping.required && <Typography variant="caption" sx={{
                  bgcolor: 'error.light',
                  px: 1,
                  borderRadius: 1
                }}>
                          ...
                          {mapping.description && <Typography variant="caption" display="block">
                              {mapping.description}
                            </Typography>}
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="md" fullWidth>
          ...
          <Grid container spacing={3}>
            <Grid item xs={12} sm={5}>
              <FormControl fullWidth margin="normal" error={!!errors.source_field}>
                ...
                <Select name="source_field" value={currentMapping.source_field} onChange={handleMappingChange} label="Source Field" disabled={loading.sourceFields}>

                  {sourceFields.map(field => <MenuItem key={field.name} value={field.name}>
                      <Typography>
                        {field.name}
            ...
            <Grid item xs={12} sm={5}>
              <FormControl fullWidth margin="normal" error={!!errors.destination_field}>
                ...
                <Select name="destination_field" value={currentMapping.destination_field} onChange={handleMappingChange} label="Destination Field" disabled={loading.destinationFields}>

                  {destinationFields.map(field => <MenuItem key={field.name} value={field.name}>
                      <Typography>
                        {field.name}
                        <Typography variant="caption" color="text.secondary" component="span" sx={{
                      ml: 1
            ...
            <Grid item xs={12}>
              <FormControl fullWidth margin="normal">
                ...
                <Select name="transformation" value={currentMapping.transformation} onChange={handleMappingChange} label="Transformation">

                  {TRANSFORMATION_TYPES.map(option => <MenuItem key={option.value} value={option.value}>{option.label}</MenuItem>)}
            ...
            <Grid item xs={12}>
              <TextField label="Description (Optional)" name="description" value={currentMapping.description} onChange={handleMappingChange} fullWidth multiline rows={2} margin="normal" />
            ...
            <Grid item xs={12}>
              <FormControl>
                <label>
                  <input type="checkbox" name="required" checked={currentMapping.required} onChange={handleMappingChange} />
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleSaveMapping} color="primary" disabled={loading.saving}>
...
export default withErrorBoundary(FieldMappingEditor, {
  boundary: 'FieldMappingEditor',
  fallback: ({
    error,
    resetError
  }) => <Paper sx={{
    ...
    color: 'error.contrastText'
      ...
      <Typography variant="h6" gutterBottom>Field Mapping Error</Typography>
      <Typography variant="body1" sx={{
      mb: 2
    }}>
        {error?.message || 'An error occurred while managing field mappings.'}
      </Typography>
      <Button variant="outlined" color="inherit" size="small" onClick={resetError}>
