  ...
  return <Paper elevation={selected ? 3 : 1} sx={{
    ...
    position: 'relative'
      ...
      borderTopRightRadius: 4
        ...
        <TransformationIconComponent fontSize="small" sx={{
        mr: 1
      }} />
      ...
      p: 1
        ...
        {data.fields && data.fields.length > 0 && <Box sx={{
        display: 'flex',
        flexWrap: 'wrap',
        gap: 0.5,
        my: 0.5
      }}>
            <Chip label={`${data.fields.length} field${data.fields.length > 1 ? 's' : ''}`} size="small" variant="outlined" color="primary" />
...
export default withErrorBoundary(TransformationNode, {
  boundary: 'TransformationNode'
});