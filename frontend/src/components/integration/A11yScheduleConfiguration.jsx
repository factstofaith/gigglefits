...
import { Box, Chip, FormControl, FormHelperText, Grid, IconButton, InputLabel, MenuItem, Select, TextField, Tooltip, Typography } from '@mui/material';
...
const A11yScheduleConfiguration = ({
  ...
  return <Box sx={{
    mt: 2
  }} role="region" aria-labelledby="schedule-configuration-heading">

      <Typography variant="subtitle1" gutterBottom id="schedule-configuration-heading" tabIndex={-1}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} md={6}>
          <FormControl fullWidth error={!!errors.type}>
            ...
            <Select labelId="schedule-type-label" id="schedule-type" name="type" value={currentSchedule.type} onChange={handleChange} label="Schedule Type" disabled={readOnly} {...getFieldA11yAttributes('type', 'Schedule Type', true)}>
            ...
            {errors.type && <FormHelperText id="type-error" role="alert">
                {errors.type}
              </FormHelperText>}
        ...
        {!isOnDemand && <Grid item xs={12} md={6}>
            <FormControl fullWidth error={!!errors.timezone}>
              ...
              <Select labelId="timezone-label" id="timezone" name="timezone" value={currentSchedule.timezone} onChange={handleChange} label="Timezone" disabled={readOnly} {...getFieldA11yAttributes('timezone', 'Timezone', true)}>

                {timezones.map(tz => <MenuItem key={tz} value={tz}>{tz}</MenuItem>)}

              </Select>
              {errors.timezone && <FormHelperText id="timezone-error" role="alert">
                  {errors.timezone}
                </FormHelperText>}
        ...
        {(isCustom || currentSchedule.cronExpression) && !isOnDemand && <Grid item xs={12}>
          ...
          gap: 1
              ...
              {isEditingCron ? <TextField fullWidth inputRef={cronInputRef} name="cronExpression" id="cronExpression" label="Cron Expression" value={currentSchedule.cronExpression} onChange={handleChange} onBlur={handleCronBlur} error={!cronValid || !!errors.cronExpression} helperText={errors.cronExpression || "Format: sec min hour day month weekday (e.g., '0 0 2 * * *' for daily at 2am)"} disabled={readOnly} autoFocus aria-describedby="cron-format-help" {...getFieldA11yAttributes('cronExpression', 'Cron Expression', isCustom)} /> : <>
                  <Box sx={{
              flexGrow: 1
            }}>
                ...
                gap: 1
                      ...
                      <Chip label={currentSchedule.cronExpression || "None"} variant="outlined" color={cronValid ? "primary" : "error"} aria-labelledby="cron-label" />

                      {!readOnly && <A11yButton size="small" onClick={handleEditCron} a11yLabel="Edit cron expression" a11yAnnouncement="Editing cron expression" variant="outlined" startIcon={<EditIcon fontSize="small" />}>

                          Edit
                        </A11yButton>}

                      <Tooltip title="Cron expressions define the schedule pattern using 6 fields: second minute hour day month weekday">
                        <IconButton size="small" aria-label="Cron expression format information">
                ...
                mt: 0.5
              }} aria-live="polite" id="cron-human-readable">
        ...
        {!isOnDemand && <Grid item xs={12} md={6}>
            <FormControl fullWidth error={!!errors.effectiveDate}>
              <TextField name="effectiveDate" id="effectiveDate" label="Effective From (Optional)" type="datetime-local" value={currentSchedule.effectiveDate} onChange={handleChange} InputLabelProps={{
            shrink: true
          }} disabled={readOnly} error={!!errors.effectiveDate} helperText={errors.effectiveDate || "Leave blank to start immediately"} {...getFieldA11yAttributes('effectiveDate', 'Effective From')} aria-describedby="effective-date-help" />
        ...
        <Grid item xs={12}>
          <FormControl fullWidth>
            <TextField name="description" id="scheduleDescription" label="Schedule Description (Optional)" value={currentSchedule.description} onChange={handleChange} multiline rows={2} disabled={readOnly} helperText="Add notes about this schedule for other team members" {...getFieldA11yAttributes('description', 'Schedule Description')} />
