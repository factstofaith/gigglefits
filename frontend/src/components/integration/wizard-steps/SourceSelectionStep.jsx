 ...
 */
const SourceSelectionStep = ({ formik }) => {
  ...
  return (
    <Box>
            ...
            <Grid container spacing={3}>
              {sourcesByCategory[category].map((source) => (
                <Grid item xs={12} sm={6} md={4} key={source.id}>
                  <Card 
                    elevation={formik.values.sourceType === source.id ? 3 : 1}
                    sx={{
                      height: '100%',
                      ...
                      <CardContent>
                        <Box sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
                          <FormControlLabel
                            control={
                              <Radio
                                checked={formik.values.sourceType === source.id}
                                value={source.id}
                                name="sourceType"
                              />
                            }
      ...
      {formik.touched.sourceType && formik.errors.sourceType && (
        <FormHelperText error>{formik.errors.sourceType}</FormHelperText>
      )}
    </Box>
  );
};
