  ...
  const renderSourceConfig = () => {
          ...
          return (
            <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                ...
                <Typography variant="body1">
                  {values.config.useManagedIdentity
    ...
    return (
      <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
        ...
        <Box sx={{ mt: 2 }}>{renderSourceDetails()}</Box>
      </Paper>
    );
  };
  ...
  const renderSchemaConfig = () => (
    <Paper variant="outlined" sx={{ p: 2, mb: 3 }}>
      ...
      <Grid container spacing={2}>
        ...
        {values.schemaDefinitionMethod === 'manual' && values.schema.length > 0 && (
          <Grid item xs={12}>
            <Typography variant="subtitle2" color="text.secondary">
              Fields
            </Typography>
            <List dense>
              {values.schema.map((field, index) => (
                <ListItem key={index}>
                  <ListItemIcon>
        ...
        {values.schemaDefinitionMethod === 'auto' && (
          <Grid item xs={12}>
            <Typography variant="subtitle2" color="text.secondary">
              Sample Data
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {values.sampleData
                ? `${values.sampleData.substring(0, 100)}...`
                : 'No sample data provided'}
            </Typography>
          </Grid>
        )}

        {values.schemaDefinitionMethod === 'upload' && (
          <Grid item xs={12}>
            <Typography variant="subtitle2" color="text.secondary">
              Schema File
            </Typography>
