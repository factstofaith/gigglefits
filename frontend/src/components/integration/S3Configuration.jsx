...
const S3Configuration = ({
  ...
  return <Box>
      ...
      <Accordion defaultExpanded>
        ...
        <AccordionDetails>
          <S3CredentialManager onCredentialsSaved={handleCredentialsChange} onCredentialsLoaded={handleCredentialsChange} readOnly={readOnly} showSaveControls={true} initialVisible={true} />

        </AccordionDetails>
      </Accordion>

      {/* Bucket and Object Selection */}
      <Accordion expanded={showBrowser} onChange={() => setShowBrowser(!showBrowser)} disabled={!config.credentials}>

        <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls="browser-content" id="browser-header">
          ...
          mr: 2
            ...
            {config.bucket && <Chip label={config.bucket} color="primary" size="small" variant="outlined" onClick={e => {
            e.stopPropagation();
            setShowBrowser(true);
          }} />}
        ...
        <AccordionDetails>
          {config.credentials ? <S3BucketBrowser config={browserConfig} onSelectBucket={handleBucketSelect} onSelectObject={handleObjectSelect} readOnly={readOnly} selectedBucket={config.bucket} selectedPrefix={config.path ? config.path.substring(0, config.path.lastIndexOf('/') + 1) : ''} /> : <Alert severity="warning">
      ...
      mt: 3
        ...
        <CardHeader title="Configuration Summary" action={<Button variant="outlined" color="primary" disabled={!config.credentials || !config.bucket || loading || readOnly} onClick={testConnection} startIcon={loading ? <CircularProgress size={20} /> : <RefreshIcon />} size="small">
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              ...
              mb: 2
                ...
                <Chip label={config.credentials ? "Configured" : "Not Configured"} color={config.credentials ? "success" : "default"} size="small" icon={config.credentials ? <CheckCircleIcon /> : <InfoIcon />} />
            ...
            <Grid item xs={12} md={6}>
              ...
              {config.selectedObject && <Box>
                ...
                p: 2
                  ...
                  return <FileCard file={fileObj} metadata={metadata} showPreview={false} variant="outlined" />;
                })()}
                  </Paper>
                </Box>}
              ...
              {/* Connection Status */}
              {connectionStatus && <Box sx={{
              mt: 2
            }}>
                  <Typography variant="subtitle2" gutterBottom>
                    Connection Test Result
                  </Typography>
                  <Alert severity={connectionStatus.success ? "success" : "error"} icon={connectionStatus.success ? <CloudDoneIcon /> : <ErrorIcon />}>

                    <Typography variant="body2">
                      {connectionStatus.message}
                    </Typography>
                    
                    {connectionStatus.details && connectionStatus.success && <Box sx={{
                  mt: 1,
                  fontSize: '0.875rem'
                }}>
