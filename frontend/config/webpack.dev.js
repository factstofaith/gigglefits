...
module.exports = {
  ...
  devServer: {
    ...
    // Configure file watching for Docker volumes - UPDATED for webpack-dev-server v4+
    watchFiles: {
      // Set paths to watch
      paths: ['src/**/*', 'public/**/*'],
      options: {
        ...
        // Force polling for Docker environment detection (multiple fallbacks)
        ...(process.env.DOCKER === 'true' ? { poll: true } : {}),
        ...(process.env.CHOKIDAR_USEPOLLING === 'true' ? { poll: true } : {}),
        // Special case for TAP Integration Platform containers to ensure compatibility
        ...(process.env.RUNNING_IN_DOCKER === 'true' ? { poll: true } : {}),
      }
    },
    ...
    // Setup dev middleware - removed watch property as it's not supported in webpack-dev-middleware v5
    devMiddleware: {
