...
# First try modern Docker Compose (plugin)
if docker compose version > /dev/null 2>&1; then
  DOCKER_COMPOSE_CMD="docker compose"
  print_success "Using Docker Compose plugin"
# Then try legacy Docker Compose (standalone)
elif command -v docker-compose > /dev/null 2>&1; then
  DOCKER_COMPOSE_CMD="docker-compose"
  print_success "Using standalone Docker Compose"
else
...
${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml down 2>/dev/null || true
...
if [ -f docker-compose.master.yml ]; then
  print_status "Found docker-compose.master.yml, using it as the standard configuration"
  cp docker-compose.master.yml docker-compose.yml
else
  print_warning "docker-compose.master.yml not found, using existing docker-compose.yml"
fi
...
${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml build
...
echo ""
echo "To stop the environment: ${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml down"
echo "=========================================================================="

# Ask if user wants to see logs
read -p "Do you want to follow the container logs? (y/n): " SHOW_LOGS
if [[ $SHOW_LOGS == "y" || $SHOW_LOGS == "Y" ]]; then
  ${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml logs -f
fi