...
if command -v docker &> /dev/null; then
    ...
    if docker info &> /dev/null; then
        ...
        if docker compose version &> /dev/null; then
            ...
            echo -e "Docker Compose version: $DOCKER_COMPOSE_VERSION (plugin)"
            DOCKER_COMPOSE_CMD="docker compose"
        elif command -v docker-compose &> /dev/null; then
            DOCKER_COMPOSE_VERSION=$(docker-compose --version | awk '{print $3}' | sed 's/,//')
            echo -e "Docker Compose version: $DOCKER_COMPOSE_VERSION (legacy)"
            DOCKER_COMPOSE_CMD="docker-compose"
        else
...
${DOCKER_COMPOSE_CMD} -f docker-compose-test.yml down || true
...
echo "${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml up"
echo ""
echo "For cleanup, run:"
echo "${DOCKER_COMPOSE_CMD} -f docker-compose.master.yml down"