/**
 * CoverageViewer Story
 * 
 * Visualizes code coverage with interactive heatmaps
 */
import React from 'react';
import { Story, Meta } from '@storybook/react';

import CoverageViewer from '../components/codeQuality/CoverageViewer';

export default {
  title: 'Code Quality/CoverageViewer',
  component: CoverageViewer,
  argTypes: {
    // Control definitions will be added during enhancement
  },
} as Meta;

const Template: Story = (args) => <CoverageViewer {...args} />;

export const Default = Template.bind({});
Default.args = {
  // Default props will be added during enhancement
};

export const WithData = Template.bind({});
WithData.args = {
  // Data example props will be added during enhancement
};

export const WithError = Template.bind({});
WithError.args = {
  // Error state props will be added during enhancement
  error: 'Example error message'
};
