 ...
 */
const PerformanceMetricsDisplay = memo(props => {
  const {
    children,
    className,
    id,
  ...
  const { trackRender, trackInteraction } = usePerformanceTracking('PerformanceMetricsDisplay');
  ...
  if (!isReady) {
    return (
      <div className="loading-placeholder" data-testid="loading-placeholder">
        Loading PerformanceMetricsDisplay...
      </div>
    );
  }

  return (
    <ErrorBoundary fallback={<div>Error loading PerformanceMetricsDisplay</div>}>
...
PerformanceMetricsDisplay.displayName = 'PerformanceMetricsDisplay';
